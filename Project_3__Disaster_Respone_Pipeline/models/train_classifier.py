...
def load_csv(csv_filepath):
    '''Loads the data from the csv (for Heroku, which won't accept SQLlite)
    Args:
        csv_filepath (str) - path to csv
    Returns:
        X (pd.DataFrame) - the messages to classify
        Y (pd.DataFrame) - the labelled classifications
        category_names (python list) - the categories of classifications
    '''
    df = pd.read_csv(csv_filepath, index_col=0, header=0, dtype = 'str' )
    df.iloc[:,3:] = df.iloc[:,3:].astype(int) # Change bool columns to bool
    X = df['message']
    Y = df.iloc[:,3:]
    category_names = Y.columns
    return X, Y, category_names
...
def build_model():
    ...
    Returns:
        (sklearn Estimator)
    '''
    # The final classifier, SGDC(loss='modified_huber') chosen from gridsearch, see notebook for details
...
def evaluate_model(model, X_test, Y_test, category_names, verbose = False):
    ...
    Args:
        ...
        category_names (python list) - the categories of classifications
        verbose (Bool) - if True, print the full classification report
    Returns:
        None
    '''
    Y_pred = model.predict(X_test)
    print('\tf1_score %0.2f' % f1_score(Y_pred, Y_test, average='weighted'))
    if verbose: # Print full classification report for each category
...
def main():
    ...
    if len(sys.argv) == 3:
        ...
        print('Loading data...\n    FILE: {}'.format(database_filepath))
         
        # Load data
        X, Y, category_names = load_data(database_filepath) # For local DB
        #X, Y, category_names = load_csv(database_filepath)  # For Heroku
